...
POE::Session->create(
  package_states => [
	'main' => [qw(_start _launch _response)],
  ],
);
...
sub _response {
  ...
  ok( $res->{data} eq 'NRPE: Command timed out after 5 seconds', 'And the data was cool' ) or diag("Got '$res->{data}', expected 'NRPE: Command timed out after 5 seconds'\n");
  $nrped->shutdown();
  return;
}
