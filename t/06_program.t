...
sub _start {
  ...
  ok( $nrped->add_command( command => 'check_mofo', program => [ $^X, 'check_mofo.pl' ] ), "Added a command handler okay" );
...
sub _response {
  ...
  ok( $res->{data} eq 'OK mofo bleh' ) or diag("Got '$res->{data}', expected 'OK mofo bleh'\n");
  $nrped->shutdown();
  return;
}
